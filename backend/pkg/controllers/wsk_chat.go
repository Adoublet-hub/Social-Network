...
func (s *MyServer) OnlineUsersHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		log.Printf("Users currently online: %v", s.WebSocketChat.Users)
		...
		log.Println("Online users: ", len(users))
...
func (s *MyServer) GetMessagesHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		query := `SELECT id, sender_username, target_username, content, timestamp, type, emoji FROM chatHistory WHERE sender_username = ? OR target_username = ? ORDER BY timestamp DESC LIMIT 10 OFFSET ?`
...
func (s *MyServer) PostMessageHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		s.WebSocketChat.Mu.Lock()
		defer s.WebSocketChat.Mu.Unlock()
		...
		query := `INSERT INTO chatHistory (id, sender_username, target_username, content, timestamp, type, emoji) VALUES (?, ?, ?, ?, ?, ?, ?)`
		...
		// le message au destinataire via WebSocket
		...
		// le message à l'expéditeur via WebSocket
